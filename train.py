...
model = get_model()
mcp = tf.keras.callbacks.ModelCheckpoint(monitor='val_sparse_categorical_accuracy', filepath='best.hdf5', verbose=1, save_best_only=True)
...
try:
    model.fit(x=X_train, y=y_train, validation_data=(X_val, y_val), batch_size=config['batch_size'], epochs=config['epochs'], callbacks=[mcp], use_multiprocessing=True, shuffle=True)
finally:
    pass
    #model.evaluate(x=X_val, y=y_val, batch_size=config['batch_size'], use_multiprocessing=True)
